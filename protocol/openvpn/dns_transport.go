...
func (t *DNSTransport) createResolver(server ovpntransport.DNSServer, address netip.AddrPort) (adapter.DNSTransport, error) {
	...
	return dnsTransport.NewHTTPSRaw(t.TransportAdapter, t.logger, t.dialer, destination, http.MethodGet, http.Header{}, M.SocksaddrFrom(address.Addr(), port), tlsConfig), nil
}
