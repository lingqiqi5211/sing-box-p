...
!!! warning "Linux 6.6 LPM trie 兼容性"
    ...
    panic。默认 `shared` 的本机地址策略使用精确匹配 HASH map，
...
### 从早期 1.14.0 eBPF 原型迁移

cilium/ebpf 后端替换了早期 1.14.0 开发版本中的 eBPF 原型，其配置结构不向后
兼容；旧字段名必须删除，当前版本会拒绝而不是忽略它们。
...
旧 `cgroup_enabled` 默认值为 `true`，`shared_network.enabled` 默认值为
`false`。因此，只显式启用 `shared_network` 的旧配置实际使用两条数据路径，
应迁移为 `hybrid`；同时禁用两条数据路径在旧版中属于无效配置。

将旧本机字段（`cgroup_path`、`cgroup_ipv6_mode`、UID 范围、Android 用户及包名
列表）移入 `local`，并将 `cgroup_ipv6_mode` 改名为 `local.ipv6_mode`。
将 `shared_network` 改名为 `shared`，将 `include_interface` 改为
`shared.interface`，并将 `tc_priority` 移到 `shared.advanced.tc_priority`。

三个本机 `map_capacity` 值合并为一个 `local.state_capacity`；三个 shared
容量值合并为一个 `shared.state_capacity`。值为 `0` 时使用当前默认值 65536。
如果旧配置为不同 map 设置了不同容量，应选择一个能满足最大 map 的统一容量。
旧容量字段不允许显式设置为 `0`。

`redirect_address` 已移除，sing-box 现在会自动分配不冲突的重定向前缀，旧自定义
前缀无法原样保留。为了保持地址族行为，应先检查旧 `redirect_address` 是否包含
IPv6 前缀：若不包含，应设置 `local.ipv6_mode: "off"`，并在启用 shared 路径时
设置 `shared.ipv6_mode: "off"`；若包含，则将旧 `cgroup_ipv6_mode` 的值迁移到
`local.ipv6_mode`（旧字段省略时等同于 `always`），shared 路径使用当前默认的
`always` 即可保持旧行为。

`network`、`udp_timeout`、`dns_mode`、`bypass_private_address` 和
`bypass_rule_set` 仍位于顶层；其中 `bypass_rule_set` 并未删除。使用默认值时，
local 模式会同时启用 TCP/UDP、使用 `5m` UDP 超时、劫持 DNS、使用自动探测的
cgroup v2 根目录，并采用 `local.ipv6_mode: "auto"`，因此这些值均可省略。
上述省略规则适用于新配置；迁移旧配置时应按前述 `redirect_address` 规则处理
IPv6 模式。
