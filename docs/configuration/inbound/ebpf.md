...
!!! warning "Linux 6.6 LPM trie compatibility"
    ...
    trie. The default `shared` host-address policy uses exact-match
...
### Migration from the earlier 1.14.0 eBPF prototype

The cilium/ebpf backend replaces the eBPF prototype shipped in earlier 1.14.0
development builds. Its configuration structure is not backward compatible;
old field names must be removed because they are rejected rather than ignored.

Choose `mode` from the old data-path switches:

| Earlier configuration | Current configuration |
|-----------------------|-----------------------|
| `cgroup_enabled: true`, `shared_network.enabled: false` | `mode: "local"` |
| `cgroup_enabled: false`, `shared_network.enabled: true` | `mode: "shared"` |
| Both enabled | `mode: "hybrid"` |

The old `cgroup_enabled` default was `true` and `shared_network.enabled`
defaulted to `false`. Therefore, an old configuration that only enabled
`shared_network` used both paths and should migrate to `hybrid`. Disabling both
paths was invalid.

Move the old local fields (`cgroup_path`, `cgroup_ipv6_mode`, UID ranges, Android
users, and package lists) into `local`; rename `cgroup_ipv6_mode` to
`local.ipv6_mode`. Rename `shared_network` to `shared`, rename
`include_interface` to `shared.interface`, and move `tc_priority` to
`shared.advanced.tc_priority`.

The three local `map_capacity` values are replaced by one
`local.state_capacity`; the three shared-network capacity values are replaced
by one `shared.state_capacity`. A value of `0` uses the current default of
65536. If the old per-map values differed, choose a single capacity suitable
for the largest map. The old capacity fields did not accept an explicit `0`.

`redirect_address` has been removed because sing-box now allocates
non-conflicting redirect prefixes automatically; custom prefixes cannot be
preserved. To preserve address-family behavior, first check whether the old
`redirect_address` contained an IPv6 prefix. If it did not, set
`local.ipv6_mode: "off"` and, when the shared path is enabled,
`shared.ipv6_mode: "off"`. If it did, carry the old `cgroup_ipv6_mode` value to
`local.ipv6_mode` (an omitted old value meant `always`); the current shared
default of `always` preserves the old shared-path behavior.

`network`, `udp_timeout`, `dns_mode`, `bypass_private_address`, and
`bypass_rule_set` remain at the top level. In particular, `bypass_rule_set` has
not been removed. With default values, local mode enables TCP and UDP, uses a
`5m` UDP timeout, hijacks DNS, uses the detected cgroup v2 root, and selects
`local.ipv6_mode: "auto"`; those values may be omitted for a new configuration.
